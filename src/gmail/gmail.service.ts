...
export class GmailService {
    ...
    try {
      ...
      try {
        const response = await gmail.users.messages.list({
          ...
          pageToken: defaultOptions.pageToken || undefined,
        });
        ...
        return {
          messages: messages.filter((msg): msg is NonNullable<typeof msg> =>
            msg !== null
          ),
  ...
  parseGmailMessage(message: gmail_v1.Schema$Message) {
    ...
    try {
      ...
      // Extract headers
      const subject = headers.find((h) =>
        h.name?.toLowerCase() === "subject"
      )?.value ||
        "(No subject)";
      const from = headers.find((h) =>
